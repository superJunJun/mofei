//
//  TYDCookbookParticularViewController.m
//  Mofei
//
...
@interface TYDCookbookParticularViewController ()<UIAlertViewDelegate>
@property (strong, nonatomic) NSMutableArray *cookbookDetailInfos;
@property (strong, nonatomic) NSDictionary *DIC;
@end

@implementation TYDCookbookParticularViewController
...
-(void)userDidRead
{
    NSString *key = [NSString stringWithFormat:@"id%@IsAttentionBreastCookbook",_cookbookModel.cookbookID];
    ...
    if([self.delegate respondsToSelector:@selector(cookbookParticularViewControllerCookbookModelUpdated:)])
    {
        [self.delegate cookbookParticularViewControllerCookbookModelUpdated:self];
    }
}
...
- (void)loadData
{
    ...
    [params setValue:[NSString stringWithFormat:@"%ld",(long)_detailVCModelId] forKey:@"recipeId"];
    
    [self showProgressHUDWithLabelText:nil];
    [self postURLRequestWithMessageCode:ServiceMsgCodeBreastCookeeParticular HUDLabelText:nil params:params completeBlock:^(id result) {
...
//    NSLog(@"loadDataComplete:%@", result);
    ...
    if(errorCode.intValue == 0)
    {
        ...
        if([result isKindOfClass:NSDictionary.class])
        {
            ...
            _DIC = dic[@"breastRecipesDetail"];
            NSArray * plistArray=_DIC[@"breastRecipesList"];
            for(NSDictionary *dInPlist in plistArray)
            {
                TYDBreastDetailModel *cookbookDetail = [TYDBreastDetailModel new];
                [cookbookDetail setAttributes:dInPlist];
                [_cookbookDetailInfos addObject:cookbookDetail];
                NSLog(@"小图：%@",cookbookDetail.paragraphImgUrl);
            }
        }
    }
    else
...
- (CGFloat)loadPraiseButton:(CGFloat)startY
{
    ...
    NSLog(@"%f",baseView.center.y);
    [baseView addSubview:praiseButton];
    NSLog(@"%@",baseView.subviews);
...
-(void)loadPraiseButtonInfoComplete:(id)result withModel:(TYDBreastListModel*)model
{
    ...
    {
        UIImageView * view=(UIImageView * )[self.view viewWithTag:3001];
        if(_attentionMark==0)
        {
