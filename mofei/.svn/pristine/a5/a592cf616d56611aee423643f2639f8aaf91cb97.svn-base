...
#define sDefaultAvatarName          @"common_avatarDefault"//用于未登录
#define sDefaultAvatarNameFemale    @"me_avatar3"
#define sDefaultAvatarNameMale      @"me_avatar4"
...
//#define sAllAvatarNames             @[@"me_avatar1", @"me_avatar2", \
                                    @"me_avatar3", @"me_avatar4", \
                                    @"me_avatar5", @"me_avatar6", \
                                    @"me_avatar7", @"me_avatar8"]
#define sAllAvatarNames             @[@"me_avatar3", @"me_avatar5", \
                                    @"me_avatar7", @"me_avatar4", \
                                    @"me_avatar6", @"me_avatar8"]
...
- (instancetype)initWithAvatarRadius:(CGFloat)radius
{
    ...
    if(self = [super initWithFrame:frame])
    {
        ...
        self.image = [UIImage imageNamed:sDefaultAvatarName];
...
{
    [self.avatarIcon setImageWithURLString:urlString placeholderImage:[UIImage imageNamed:sDefaultAvatarName]];
}

- (void)setAvatarImageWithAvatarID:(NSNumber *)avatarID
{
    NSString *avatarName = [self.class avatarNameWithAvatarID:avatarID];
...
- (void)setImage:(UIImage *)image
{
    if(!image)
    {
        self.avatarIcon.image = [UIImage imageNamed:sDefaultAvatarName];
...
{
    return sAllAvatarNames;
}

+ (NSString *)avatarNameWithAvatarID:(NSNumber *)avatarID
{
    uint index = avatarID.unsignedIntValue;
    NSArray *avatarNames = sAllAvatarNames;
    NSString *imageName = sDefaultAvatarNameFemale;
    if(index < avatarNames.count)
    {
        imageName = avatarNames[index];
    }
    return imageName;
}
...
{
    return isFemale ? sDefaultAvatarNameFemale : sDefaultAvatarNameMale;
}
