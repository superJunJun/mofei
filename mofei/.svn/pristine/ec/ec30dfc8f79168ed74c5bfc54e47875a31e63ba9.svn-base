//
//  TYDBreastDrillDetailViewController.m
//  Mofei
//
...
@interface TYDBreastDrillDetailViewController ()<UIAlertViewDelegate>
...
@implementation TYDBreastDrillDetailViewController
...
-(void)userDidRead
{
    NSString * key =[NSString stringWithFormat:@"id%@IsAttentionBreastDrill",_breastDrillModel.cookbookID];
    ...
    if([self.delegate respondsToSelector:@selector(breastDrillViewControllerBreastDrillModelUpdated:)])
    {
        [self.delegate breastDrillViewControllerBreastDrillModelUpdated:self];
    }
}
- (void)loadData
{
    NSMutableDictionary *params = [NSMutableDictionary new];
    [params setValue:[NSString stringWithFormat:@"%ld",(long)_detailVCModelId] forKey:@"meixiongId"];
    
    [self showProgressHUDWithLabelText:nil];
    [self postURLRequestWithMessageCode:ServiceMsgCodeBreastDrillDetail HUDLabelText:nil params:params completeBlock:^(id result) {
...
//    NSLog(@"loadDataComplete:%@", result);
    ...
    if(errorCode.intValue == 0)
    {
        ...
        if([result isKindOfClass:NSDictionary.class])
        {
            ...
            _DIC = dic[@"breastGymDetail"];
            NSArray * plistArray=_DIC[@"breastGymList"];
            for(NSDictionary *dInPlist in plistArray)
            {
                TYDBreastDetailModel *cookbookDetailModel = [TYDBreastDetailModel new];
                [cookbookDetailModel setAttributes:dInPlist];
                [_breastDrillDetailInfos addObject:cookbookDetailModel];
                NSLog(@"---->%@",cookbookDetailModel.paragraphImgUrl);
            }
        }
    }
    else
...
-(CGFloat)loadEveryStepPictureAndExplain:(float)startY
{
    ...
    for(int i=0;i<[self.breastDrillDetailInfos count];i++)
    {
        ...
        if(model.paragraphContent)
        {
            ...
            CGSize size=[BOAssistor string:text sizeWithFont:textFont constrainedToWidth:self.view.bounds.size.width-20 lineBreakMode:NSLineBreakByWordWrapping];
...
-(void)praiseButtonTapAction
{
    ...
    [self postURLRequestWithMessageCode:ServiceMsgCodePraiseButton HUDLabelText:nil params:params completeBlock:^(id result) {
        [self loadPraiseButtonInfoComplete:result withModel:_breastDrillModel];
        NSLog(@"%@",result);
    }];
}
...
- (void)loadPraiseButtonInfoComplete:(id)result withModel:(TYDBreastListModel*)model
{
    ...
    {
        UIImageView * view=(UIImageView * )[self.view viewWithTag:2001];
        if(_attentionMark==0)
        {
